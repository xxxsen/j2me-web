...
本报告对应 `j2me-web 0.3.1` 及 README 中固定的三个 fork 提交。测试环境为 Linux x86_64、Google Chrome、WebAssembly pthread、SwiftShader WebGL2 和 Web Audio。结果只代表实际走过的路径，不代表整个游戏已通关。
...
| 仙剑奇侠传完美版 | 通过；MIDP 1.0 原生 176×208 画面持续渲染 | Enter 端到端映射为 FIRE `-5`，输入后画面继续推进 | 启动路径未触发媒体播放 | `startApp()` 长驻游戏循环已隔离到 MIDlet 线程，不再阻塞 Chrome/GLFW 主循环 |
...
`0.3.1` 的公共 API（checkpoint ABI 自 `0.1.0` 未变）沿用同一组 1202 B `j2me-rms-bundle-v1` 回归证据（额外字节为魔数、游戏 SHA-256、路径和长度元数据），可在新的页面/runtime 实例中于 MIDlet 启动前导入。需要强调：该结果证明了宿主保存数据的跨实例传递，不代表任意 MIDlet 都会跳过自身标题/读档菜单自动回到保存画面。

## 启动性能

miniJVM 对已打开 JAR reader 做类加载期复用，Wasm 使用 `-O3 -msimd128`。`0.3.1` 最终候选版在同一 Chrome/《魔塔》的三次隔离 context 中，READY 为 3123、2575、2629 ms，中位数 2629 ms；前 5 帧中位数 3024 ms。测试用第一次真实方向键确认游戏启动动画后已可交互，可交互启动中位数 10,152 ms，随后 Enter 的最差端到端延迟 332 ms。
...
`0.3.1` 的 Chrome 回归要求至少 3 个完成周期、至少一次实际回收、GC 后 Java 堆维持在观察下限加 16 MiB 内、真实 STW 不超过 2 秒，并在每轮之间检查帧与输入。独立 `test:gc` 完成 3 轮并累计回收 106,570,986 B，GC 后 Java 堆为 34,454,171、35,327,117、35,640,340 B，最大 STW 4 ms、锁等待 0 ms。

默认 15 分钟 Chrome 浸泡共推进 34,355 帧、完成 51 个 GC 周期和 14 次 PNG 截图，最大真实 STW 92 ms；Chrome JS 堆在 2.8–4.1 MiB 之间波动并回落。第 5 分钟的暂停/恢复后，逐次输入仍持续进入 MIDlet 队列；全程没有 GC 取消、协调锁超时、OOM、Wasm trap、崩溃或卡死。
